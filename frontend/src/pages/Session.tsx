...
const Session: React.FC = () => {
  ...
  return (
    <div style={{ paddingLeft: 25, paddingRight: 25, paddingBottom: 300}}>
      <h2 style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <button
          ...
          style={{ fontSize: 18, color: '#dc3545', background: 'none', border: 'none', cursor: 'pointer', marginLeft: 8 }}
      ...
      <ul>
        {session.players.map(player => {
          ...
          return (
            <li key={player.id} style={{ marginBottom: 24 }}>
              <strong>{player.name}
                <button
                  type="button"
                  onClick={async () => {
              ...
              <ul>
                {player.pokemon && player.pokemon.length > 0 ? (
                  player.pokemon.map(p => (
                    <li key={p.id} style={{ background: p.status === 'dead' ? '#ffb3b3' : p.status === 'fainted' ? '#ffe0b3' : undefined, display: 'flex', alignItems: 'center', gap: 6 }}>
                      <img
                      ...
                      {editingPokemonId === p.id ? (
                        ...
                        <>
                          {p.nickname ? `${p.nickname} (${p.species})` : p.species} - Status: {p.status}
                          <button
                            type="button"
                            style={{ background: 'none', border: 'none', padding: 2, marginLeft: 4, cursor: 'pointer', fontSize: 16 }}
              ...
              {/* Add Pokémon form */}
              <form onSubmit={e => handleAddPokemon(e, player.id)} style={{ marginTop: 8, display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', maxWidth: '50%' }}>
                {speciesLoading ? (
      ...
      {session.players.length < 2 && (
        <form onSubmit={handleAddPlayer} style={{ marginTop: 16 }}>
          <input
            type="text"
            ...
            required
          />
          <button type="submit" disabled={addingPlayer}>Add Player</button>
        </form>
      )}
      <h3>Links</h3>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: 8 }}>
          <thead>
            <tr>
              ...
              <th style={{ textAlign: 'left', borderBottom: '1px solid #ccc' }}>Location / Status</th>
            </tr>
          </thead>
          <tbody>
      ...
      {session.players.length === 2 && (
        <form onSubmit={handleLink} style={{ marginTop: 16, display: 'flex', gap: 10, alignItems: 'center', maxWidth: '50%' }}>
          <Select
            ...
            placeholder="Select Player 1 Pokémon"
            isSearchable
            components={{ Option: SpeciesOption, SingleValue: SpeciesSingleValue }}
            styles={{ menu: base => ({ ...base, zIndex: 9999 }) }}
          />
          <Select
            ...
            placeholder="Select Player 2 Pokémon"
            isSearchable
            components={{ Option: SpeciesOption, SingleValue: SpeciesSingleValue }}
            styles={{ menu: base => ({ ...base, zIndex: 9999 }) }}
          />
